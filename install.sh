...
# ------------------------------------------------------------------ path shim
# The CLI is a plain .mjs file inside the install directory, which is not on
# anybody's PATH -- so every instruction had to be spelled
# `node ~/.local/share/prmpt/bin/prmpt.mjs <cmd>`. A tiny shim fixes that, and a
# shim rather than a symlink on purpose:
#
#   * the tarball's mode bits are not something to depend on, and a symlink to a
#     non-executable file is not runnable;
#   * `prmpt update` swaps the install tree by RENAME, so anything pointing INTO
#     that tree by path keeps working, but a shim can also fall back to whatever
#     `node` is on PATH when the interpreter this installer found has moved
#     (nvm rotates its versions out from under absolute paths).
#
# It is only ever overwritten when it carries our own marker, so a `prmpt` that
# belongs to somebody else is reported rather than replaced.
SHIM_MARKER="prmpt.cash CLI shim"

default_bin_dir() {
  # Prefer a directory the user already has on PATH, so the common case needs no
  # shell-rc edit at all. ~/.local/bin is the fallback because it is the XDG
  # convention and many shells add it when it exists.
...
# Writes $BIN_DIR/prmpt and sets CLI_CMD when the result is actually callable.
link_cli() {
  [ "$LINK_CLI" -eq 1 ] || { skip "not linking \`prmpt\` (--no-path)"; return 0; }
  [ -n "$BIN_DIR" ] || BIN_DIR=$(default_bin_dir)

  shim="$BIN_DIR/prmpt"
  if [ -e "$shim" ] && ! grep -q "$SHIM_MARKER" "$shim" 2>/dev/null; then
    warn "$shim exists and is not ours -- leaving it alone."
    warn "run the CLI as: $NODE_BIN $CLI"
    return 0
  fi

  mkdir -p "$BIN_DIR" 2>/dev/null || { warn "could not create $BIN_DIR"; return 0; }
  # Written to a temporary name and renamed in, so a shim being replaced while
  # another shell is running it is never observed half-written.
  tmp="$shim.$$"
  cat > "$tmp" <<SHIM || { rm -f "$tmp"; warn "could not write $shim"; return 0; }
#!/bin/sh
# $SHIM_MARKER -- generated by install.sh. Safe to delete.
NODE="$NODE_BIN"
[ -x "\$NODE" ] || NODE=node
exec "\$NODE" "$CLI" "\$@"
SHIM
  chmod 755 "$tmp" 2>/dev/null || :
  mv -f "$tmp" "$shim" || { rm -f "$tmp"; warn "could not write $shim"; return 0; }
  ok "linked ${B}prmpt${R} -> $shim"

  case ":${PATH:-}:" in
    *":$BIN_DIR:"*)
      # On PATH, but something else may still win the name.
      found=$(command -v prmpt 2>/dev/null || echo "")
      if [ -n "$found" ] && [ "$found" != "$shim" ]; then
        warn "another prmpt comes first on your PATH: $found"
      else
        CLI_CMD="prmpt"
      fi
      ;;
    *)
      ...
      warn "then open a new shell, or run: export PATH=\"$BIN_DIR:\$PATH\""
      ;;
  esac
}
...
if [ "$UNINSTALL" -eq 1 ]; then
  ...
  # Only ever a shim we wrote: a `prmpt` belonging to something else must
  # survive an uninstall of this one.
  for d in ${BIN_DIR:-} "$HOME/.local/bin" "$HOME/bin"; do
    [ -f "$d/prmpt" ] || continue
    grep -q "$SHIM_MARKER" "$d/prmpt" 2>/dev/null || continue
    rm -f "$d/prmpt" && ok "removed $d/prmpt"
  done
...
say ""
say "${B}Linking the CLI${R}"
link_cli
# Everything printed from here on names the CLI. `prmpt` when it is genuinely
# callable, the long form when it is not -- printing a command the reader cannot
# run is worse than printing a long one.
[ -n "$CLI_CMD" ] || CLI_CMD="$NODE_BIN $CLI"
say ""
