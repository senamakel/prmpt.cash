...
.DESCRIPTION
  The PowerShell twin of install.sh. Same steps, same on-disk result:
    ...
    3. asks where to install itself, on a console; -Yes or -Agents skips it
    4. creates a wallet and signs in with it
    5. wires up every agent it chose, using that agent's own documented hook
    6. opens the setup page on the web, signed in (-NoOnboard skips it)
...
param(
  ...
  [string] $BinDir   = $env:PRMPT_BIN_DIR,
  [switch] $NoPath,
  [switch] $Project,
  [switch] $StatusLine,
  [switch] $NoOnboard,
...
# ------------------------------------------------------------------ path shim
# Where the `prmpt` command itself goes. NOT inside $Dir: `prmpt update` replaces
# that whole tree by rename, so a shim living in it would be deleted by the first
# update. Deliberately the same directory install.sh uses, so an install done
# under Git Bash and one done here do not leave two commands on one machine.
$ShimMarker = 'prmpt.cash CLI shim'
if (-not $BinDir) { $BinDir = Join-Path $Home_ '.local\bin' }
$Shim = Join-Path $BinDir 'prmpt.cmd'
# The command as the closing notes should print it -- replaced with the bare
# name once the shim is genuinely callable.
$CliCmd = $null
if ($Project) {
...
if ($Uninstall) {
  ...
  # Only ever a shim we wrote: a `prmpt.cmd` belonging to something else must
  # survive an uninstall of this one.
  if ((Test-Path $Shim) -and ((Get-Content -Raw $Shim) -match [regex]::Escape($ShimMarker))) {
    Remove-Item -Force $Shim; Write-Ok "removed $Shim"
...
# ------------------------------------------------------------------ path shim
# A .cmd rather than a symlink: creating a symlink on Windows needs either
# Developer Mode or an elevated shell, and an installer that demands elevation
# to make one command reachable is an installer people cancel.
if ($NoPath) {
  Write-Skip 'not linking prmpt (-NoPath)'
} elseif ((Test-Path $Shim) -and -not ((Get-Content -Raw $Shim) -match [regex]::Escape($ShimMarker))) {
  Write-Warn "$Shim exists and is not ours -- leaving it alone."
  Write-Warn "run the CLI as: node $Cli"
} else {
  try {
    New-Item -ItemType Directory -Force -Path $BinDir | Out-Null
    # The interpreter this installer found, with a fallback to whatever `node`
    # is on PATH -- a version manager can retire the absolute path later.
    $lines = @(
      '@echo off',
      "REM $ShimMarker -- generated by install.ps1. Safe to delete.",
      'setlocal',
      ('set "PRMPT_NODE=' + $NodeBin + '"'),
      'if not exist "%PRMPT_NODE%" set "PRMPT_NODE=node"',
      ('"%PRMPT_NODE%" "' + $Cli + '" %*')
    )
    Set-Content -Path $Shim -Value $lines -Encoding ASCII
    Write-Ok "linked prmpt -> $Shim"

    # The user PATH, read as the USER value only. Reading $env:PATH and writing
    # it back would copy the machine PATH into the user one, permanently.
    $userPath = [Environment]::GetEnvironmentVariable('PATH', 'User')
    $entries  = @(($userPath -split ';') | Where-Object { $_ })
    ...
    Write-Warn "could not write $Shim -- run the CLI as: node $Cli"
  }
}
# Printing a command the reader cannot run is worse than printing a long one.
if (-not $CliCmd) { $CliCmd = "node $Cli" }
...
if (-not $StatusLine) {
  ...
  Write-Host ''
  Write-Host "  Turn it on:   $CliCmd statusline install"
...
if ($NoOnboard -or -not (Test-Path $cfgFile)) {
  Write-Host "  Open it with:  $CliCmd onboard"
} else {
  $env:PRMPT_ENDPOINT = $Endpoint
